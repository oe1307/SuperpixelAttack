    ...
    def _attack(self, x_all: Tensor, y_all: Tensor) -> Tensor:
        ...
        for x, y in zip(x_all, y_all):
            ...
            _is_upper = torch.randint(
                0, 2, (x.shape[0], n_labels), dtype=torch.bool, device=config.device
            )
            for label in range(n_labels):
                # RGB
                is_upper[0, labels == label] = _is_upper[0, label]
                is_upper[1, labels == label] = _is_upper[1, label]
                is_upper[2, labels == label] = _is_upper[2, label]
